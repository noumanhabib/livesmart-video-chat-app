...
if (!function_exists('doctreat_change_user_email')) {
    ...
    function doctreat_change_user_email() {
		...
		$do_check = check_ajax_referer('dc_email_nonce', 'dc_email_nonce', false);
...
if ( !function_exists( 'doctreat_booking_doctor' ) ) {
	...
	function doctreat_booking_doctor() {
		...
		if( !empty( $booking_hospitals ) && !empty( $booking_slot ) && !empty( $appointment_date )) {
			...
			$contents		.= @$_SESSION['livesmart_content'];
			$booking_post 	= array(
...
if ( !function_exists( 'doctreat_booking_complete' ) ) {
	...
	function doctreat_booking_complete() {
		...
		$date_formate		= get_option('date_format');
		$time_format 		= get_option('time_format');
		if(!isset($_SESSION)){ session_start(array('user_data', 'livesmart_content', 'livesmart_content_visitor'));}
		...
		$bk_phone	= !empty( $user_data['booking']['bk_phone'] ) ? $user_data['booking']['bk_phone'] : '';
                ...
                if (get_option('livesmart_server_url')) {
                    $userdata = get_userdata($user_data['booking']['_doctor_id']);
                    $agent_user_name = $userdata->user_login;
                    $lsvTime = !empty($booking_slot) ? explode('-',$booking_slot) : array();
                    $booking = array('agent_name' => doctreat_full_name($doctor_id), 'visitor_name' => doctreat_full_name($linked_profile_id), 'date' => $appointment_date, 'start_time' => $lsvTime[0], 'end_time' => $lsvTime[1], 'agent_user' => $agent_user_name);
                    $lsv_visitor_url = $doctor_id.$appointment_date.$lsvTime[0];
                    $lsv_agent_url = $lsv_visitor_url.'_a';

                    $lsv_url = get_option('livesmart_server_url');

                    $lsvContent = '<br/><br/>Meeting agent URL: &#13;&#10;' . $lsv_url . $lsv_agent_url . '&#13;&#10;<br/>Meeting visitor URL: &#13;&#10;' . $lsv_url . $lsv_visitor_url;
                    $_SESSION['livesmart_content'] = $lsvContent;
                    $_SESSION['livesmart_content_visitor'] = '<br/>' . $lsv_url . $lsv_visitor_url;
                    insertLiveSmartRoom($booking, $lsv_agent_url, $lsv_visitor_url);
                }
		...
		if(!empty($payment_type) && $payment_type == 'offline' && empty($enable_checkout_page)){
			...
			$booking_post 	= array(
								...
								'post_type'     => 'booking',
								'post_content'	=> $contents.$lsvContent
							);
		...
		if(!empty($booking_id)) {
			...
			<div class="dc-user-steps">
				...
				<?php if( !empty( $booking_id ) && !empty( $current_user_type ) && $current_user_type != 'regular_users' ) { ?>
					<div class="dc-btnarea toolip-wrapo">
						...
						<?php if( $post_status_key === 'publish' ){?>
							<a href="<?php echo esc_url($prescription_url);?>" <?php do_action('doctreat_get_tooltip_data','element','generate_prescription');?> class="dc-btn dc-filebtn dc-tipso"><i class="ti-files"></i></a>
						<?php } ?>
