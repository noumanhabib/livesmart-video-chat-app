				...
				if( !empty($user->roles[0]) && $user->roles[0] == 'seller' ){
					...
					if(!empty($redirect_id)){
						...
						if( !empty($profile_page) ) {
							$profile_url    = Doctreat_Profile_Menu::doctreat_profile_menu_link('insights', $user->ID, true);
						}
...
if( !function_exists( 'doctreat_process_registration' ) ){
	function doctreat_process_registration(){
		...
		$do_check = check_ajax_referer('ajax-nonce', 'security', false);
        
		if ( $do_check == false ) {
            $json['type'] 		= 'error';
            $json['message'] 	= esc_html__('No kiddies please', 'doctreat_core');
            wp_send_json( $json );
        }
		...
		$location   	= !empty( $location ) ? esc_html( $location ) : '';
       	$password  		= !empty( $password ) ? esc_html( $password ) : '';
       	$user_type 		= !empty( $user_type ) ? esc_html( $user_type ) : '';
		...
		if( !empty($user_type) && $user_type == 'seller' ){
			...
			$return_page    = '';
			if( !empty($profile_page) ) {
				$return_page    = Doctreat_Profile_Menu::doctreat_profile_menu_link('insights', $user_identity, true);
			}
		}
			...
			<div class="dc-loginarea">
				<?php if( !empty( $is_login ) ) {?>
					...
					<div class="dc-loginoption">
						<a href="javascript:;" id="dc-loginbtn" class="dc-loginbtn"><?php esc_html_e('Login','doctreat_core');?></a>
						<div class="dc-loginformhold">
							...
							<form class="dc-formtheme dc-loginform do-forgot-password-form dc-hide-form">
								...
								<input type="hidden" name="dc_pwd_nonce" value="<?php echo wp_create_nonce("dc_pwd_nonce"); ?>" />
								<div class="dc-loginfooterinfo">
